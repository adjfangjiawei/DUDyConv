...
torch::Tensor fdc_forward_n128_k3_d256_cuda(
    ...
    TORCH_CHECK(kc.size(1) == FDC_FWD_N128_K, "forward_n128_k3_d256 requires K == 3.");
...
#pragma unroll
    for (int kk = 0; kk < FDC_FWD_N128_K; ++kk) {
        const float* A = kc_ptr + (int64_t)kk * FDC_FWD_N128_N * T;
    ...
    FDC_DEBUG_PATH("forward_n128_k3_d256_sgemm3_direct_bknt_combine");
