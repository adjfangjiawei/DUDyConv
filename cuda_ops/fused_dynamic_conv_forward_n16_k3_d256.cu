...
constexpr int FDC_FWD_N16_D = 256;
constexpr int FDC_FWD_N16_N = 16;
constexpr int FDC_FWD_N16_K = 3;

static inline void fdc_forward_n16_check_cublas_status(
...
static inline bool fdc_forward_n16_shape_ok(
    torch::Tensor h,
    torch::Tensor kc,
    torch::Tensor mix,
    int64_t off,
    ...
    if (h.size(1) != FDC_FWD_N16_D) {
        return false;
    }

    if (mix.size(0) != FDC_FWD_N16_D) {
        return false;
    }
...
__global__ void fdc_forward_n16_combine_noboundary_kernel(
    ...
    int64_t tmp0 = (int64_t)0 * FDC_FWD_N16_D * T + ot;
    int64_t tmp1 = (int64_t)1 * FDC_FWD_N16_D * T + ot;
    int64_t tmp2 = (int64_t)2 * FDC_FWD_N16_D * T + ot;
...
__global__ void fdc_forward_n16_combine_boundary_kernel(
    const float* __restrict__ h,
    const float* __restrict__ tmp,
    float* __restrict__ out,
    int L,
...
bool fdc_forward_n16_k3_d256_available_cuda(
    torch::Tensor h,
    torch::Tensor kc,
    torch::Tensor mix,
    int64_t off,
...
torch::Tensor fdc_forward_n16_k3_d256_cuda(
    torch::Tensor h,
    torch::Tensor kc,
    torch::Tensor mix,
    int64_t off
    ...
    TORCH_CHECK(h.size(0) == 1, "forward_n16_k3_d256 requires B == 1.");
    ...
    fdc_forward_n16_check_cublas_status(
        cublasSetStream(
            handle,
            stream
        ),
...
#pragma unroll
    for (int kk = 0; kk < FDC_FWD_N16_K; ++kk) {
        ...
        fdc_forward_n16_check_cublas_status(
            ...
            "cublasSgemm failed in forward_n16_k3_d256"
        );
    }

    int combine_total = FDC_FWD_N16_D * T;
    int combine_block = 256;
    int combine_grid = (combine_total + combine_block - 1) / combine_block;

    if (static_cast<int>(off) >= 2) {
        fdc_forward_n16_combine_noboundary_kernel<<<
            combine_grid,
            combine_block,
            0,
            stream
        ...
        fdc_forward_n16_combine_boundary_kernel<<<
            combine_grid,
            combine_block,
            0,
            stream
