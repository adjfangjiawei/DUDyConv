...
//           = mix @ kc_flat^T
//
//   gh[d,s] = sum_kk go[d,t] * w[d, kk*T+t]
//             where t = s - off + kk
// ======================================================================================
...
//
// input:
//   kc: [1,3,N,T], contiguous
//
// output:
//   kc_flat: [3*T,N]
//   kc_flat[kk*T+t, n] = kc[0,kk,n,t]
// ======================================================================================
...
__global__ void fdc_manyn_build_kc_flat_kernel(
    ...
    int64_t total = static_cast<int64_t>(3) * T * N;
...
__global__ void fdc_manyn_scatter_gk_float_kernel(
    ...
    int64_t idx = static_cast<int64_t>(blockIdx.x) * blockDim.x + threadIdx.x;
    int64_t total = static_cast<int64_t>(3) * N * T;
    ...
    int n = static_cast<int>((idx / T) % N);
    int kk = static_cast<int>(idx / (N * T));

    gk[(static_cast<int64_t>(kk) * N + n) * T + t] =
        gk_flat[static_cast<int64_t>(kk * T + t) * N + n];
}
...
static std::vector<torch::Tensor> fdc_backward_manyn_k3_d256_float(
    ...
    FDC_DEBUG_PATH("FDC path: backward_manyn_k3_d256_gemm_float_v2_bknt");
...
static std::vector<torch::Tensor> fdc_backward_manyn_k3_d256_typed(
    ...
    FDC_DEBUG_PATH("FDC path: backward_manyn_k3_d256_gemm_typed_v2_bknt");
...
std::vector<torch::Tensor> fdc_backward_manyn_k3_d256_cuda(
    ...
    TORCH_CHECK(kc.size(1) == 3, "manyn_k3_d256 requires K == 3.");
